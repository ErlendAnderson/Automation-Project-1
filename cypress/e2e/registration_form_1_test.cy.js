...
Assignment 2:
 ...
 1. Update the name of test suite by adding you name: “This is first test suite, John Smith” - DONE
 2. Replace text ‘MyPass’ in the first test with your own chosen password (2 places) - passwords should match- DONE
 3. Change phone number in the first test to 555666777- DONE
 4. Change the order of steps in the first test:- DONE
      -first set phone number- DONE
      -then 2 password fields- DONE
      -then username- DONE
 5. Add comment to the first test containing today’s date- DONE
 */

describe('This is first test suite, Erlend Anderson', () => {
    //12.06.2023
    it('User can submit data only when valid mandatory values are added', () => {
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('input[id="firstName"]').type('Erlend')
        cy.get('input[id="lastName"]').type('Anderson')
        cy.get('input[name="password"]').type('thirdeye')
        cy.get('[name="confirm"]').type('thirdeye') 
    ...
    */
    ...
    it('User cannot submit data when password and/or confirmation password is absent', () => {
        ...
        // All other fields should be entered correctly
        // Assert that submit button is not enabled and that successful message is not visibl
        cy.get('#username').type('Erlend')
        cy.get('input[name="password"]').type('thirdeye')
        cy.get('[name="confirm"]').type('  ')
        //Phone number
        cy.get('[data-testid="phoneNumberTestId"]').scrollIntoView()
        cy.get('[data-testid="phoneNumberTestId"]').clear().type('1')
        cy.get('h2').contains('Password').click()
    ...
    it('User cannot add letters to phone number', () => {
        ...
        // Assert that submit button is not enabled and that successful message is not visible
        cy.get('#username').type('ErlendAndersonJavaScript')
        cy.get('input[id="firstName"]').type('E')
        cy.get('input[id="lastName"]').type('A')
        cy.get('input[name="password"]').type('thirdeye')
        cy.get('[name="confirm"]').type('thirdeye')
        cy.get('[data-testid="phoneNumberTestId"]').should('have.attr', 'type', 'number')
        cy.get('[data-testid="phoneNumberTestId"]').clear().type('wasd')
        cy.get('h2').contains('Password').click()
